...
describe('a1111 deforum adapter', () => {
  it('translates the reviewed preset into Deforum settings', () => {
    ...
    expect(settings.scale).toBe(4.5);
    expect(settings.cfg_scale_schedule).toBe('0: (4.5)');
    expect(settings.seed_behavior).toBe('fixed');
    expect(settings.animation_mode).toBe('2D');
    expect(settings.use_depth_warping).toBe(false);
    expect(settings.strength).toBe(0.12);
    expect(settings.strength_schedule).toBe('0: (0.12)');
    expect(settings.noise_schedule).toBe('0: (0.35)');
    expect(settings.image_strength_schedule).toBe('0: (0.12)');
    expect(settings.blendFactorMax).toBe('0: (0.55)');
    expect(settings.blendFactorSlope).toBe('0: (0.35)');
    expect(settings.tweening_frames_schedule).toBe('0: (116)');
