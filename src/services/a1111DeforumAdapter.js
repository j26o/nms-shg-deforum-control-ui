...
function createControlNetDefaults(index) {
  return {
    [`cn_${index}_overwrite_frames`]: '',
    [`cn_${index}_vid_path`]: '',
    [`cn_${index}_mask_vid_path`]: '',
    [`cn_${index}_enabled`]: false,
    [`cn_${index}_low_vram`]: false,
    [`cn_${index}_pixel_perfect`]: false,
    [`cn_${index}_module`]: 'none',
    [`cn_${index}_model`]: 'None',
    [`cn_${index}_weight`]: formatNumberSchedule(1),
    [`cn_${index}_guidance_start`]: formatNumberSchedule(0),
    [`cn_${index}_guidance_end`]: formatNumberSchedule(1),
    [`cn_${index}_processor_res`]: 64,
    [`cn_${index}_threshold_a`]: 64,
    [`cn_${index}_threshold_b`]: 64,
    [`cn_${index}_resize_mode`]: 'Inner Fit (Scale to Fit)',
    [`cn_${index}_control_mode`]: 'Balanced',
    [`cn_${index}_loopback_mode`]: '',
  };
}
...
export function createDeforumRenderSettings(preset, modelOverride) {
  ...
  const settings = {
    ...
    ...createControlNetDefaults(1),
    ...createControlNetDefaults(2),
    ...createControlNetDefaults(3),
    ...createControlNetDefaults(4),
    ...createControlNetDefaults(5),
