...
- The approved Hugging Face direction is optional `huggingface-deforum`: a credential-safe Deforum-compatible backend that consumes the same simplified image-keyframe preset contract. Do not replace it with generic prompt-only text-to-video. The local proxy, frontend adapter, and private Docker Space endpoint exist; the deployed Space currently runs smoke fallback mode until it can reach a remote A1111 Deforum backend through `HF_DEFORUM_A1111_BASE_URL`. Smoke fallback crossfade MP4s are not valid creative evidence; the UI now marks the Hugging Face backend not ready when fallback mode is active and rejects fallback artifacts if they are returned.
...
- `src/config/thematicSettingPresets.js`: selectable thematic setting bundles for sample-frame matching, misty maritime night, cinematic Midjourney-like concept polish, soft watercolor memory, and fast alignment tests.
...
The current verified real backend path is a local Automatic1111 Deforum preset translator backed by the in-repo ignored `render-tools/` runtime. Runtime comparison is proven for SD 1.5, RealVisXL V5.0, and Juggernaut XL v9, and RealVisXL V5.0 is now the default model profile for new presets. The model selector includes per-model strength notes and Midjourney-fit notes; Juggernaut XL v9 is the closest configured option for a polished Midjourney-like cinematic concept-art look, while RealVisXL remains the default for source-faithful photoreal skyline atmosphere. The UI includes selectable thematic setting presets from `src/config/thematicSettingPresets.js`; `sample-frame-match` is the default and is tuned to stay closest to the supplied source images with high source strength, low denoise/noise, no depth warp, no camera drift, and longer tweening. The adapter now requires a non-empty MP4 artifact before reporting a real A1111 job as complete; settings-only Deforum outputs are treated as failed/incomplete renders. The local A1111 payload includes Deforum batch `options_overrides`, legacy prompt/scale fields, disabled ControlNet defaults, compact per-keyframe prompts, shared creative direction in `animation_prompts_positive`, and native `animation_prompts_negative` values instead of repeating a long inline `--neg` block in every local Deforum scheduled prompt. The Vite A1111 bridge also increments reused `batch_name` output folders before submission, so repeated renders with the same preset name write to separate folders such as `future-wall-morph-study-01-deforum-02`. The default source preset prioritizes source-frame retention over generative drift and the right-side Deforum controls show those values on page load with plain-language descriptions for each visible setting: each node labels its selected image as the required keyframe target, default denoise/noise are lowered, source image strength and structural lock are raised, 3D/depth warp is disabled, zoom/pan drift is removed, `locked-source-morph` is the default camera path, and the tween span is lengthened for an 8-image, 8-second, 60 fps review pass. The live Deforum batch API can reject or fail the full 8-image looper schedule with `Invalid arguments` during `PREPARING` or `Generation error` during `GENERATING`, so the Vite A1111 bridge now retries those multi-image failures by rendering adjacent two-image Deforum segments and concatenating the resulting real Deforum MP4s. A later stale-session SD 1.5 render did create an MP4, but its output drifted into abstract outlined mosaic imagery because it used the older inline prompt schedule, 3D depth warp, higher denoise/noise, and camera drift settings; do not treat that clip as evidence for the current RealVisXL source-faithful defaults. The target Windows setup should install ControlNet even when the default prototype render keeps it disabled. SDXL Base/SDXL Refiner compatibility still needs investigation. The Hugging Face path now has a local proxy, frontend adapter, and deployed private Space smoke endpoint, but no remote real Deforum MP4 render has been verified yet. The Hugging Face backend status now checks the Space `/health` response and only enables rendering when it reports `renderMode: "a1111"`, `a1111Configured: true`, and no fallback morph mode; fallback crossfade artifacts are rejected by the frontend adapter and ignored by the preview panel. A full production-grade Deforum preset exporter is still future work.
