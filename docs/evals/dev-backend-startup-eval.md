...
Date: 2026-05-19
Artifact scope: backend-aware startup flow, Vite proxy configuration, Playwright dev-server startup, README/local setup docs
Evaluator: Codex
Status: Superseded by safer startup contract

Update: `pnpm dev` is now intentionally UI-only again. The backend-aware launcher is explicit: `pnpm dev:backend`.
...
- Updated `package.json` so `pnpm dev:backend` runs the backend-aware launcher and `pnpm dev` / `pnpm dev:ui` remain frontend-only.
...
| UI/config usefulness | Pass | `pnpm dev:backend` ensures A1111 is ready and sets `VITE_SOURCE_ASSET_ROOT` before starting Vite; `pnpm dev` stays UI-only. |
...
- `pnpm dev` and `pnpm dev:ui` preserve the frontend-only Vite startup path.
...
- The full backend-aware `pnpm dev:backend` command was not executed in this eval because it can start a long-running GPU backend process.
...
The UI startup path is now safer for local reviewer use: `pnpm dev` never launches the local Deforum backend. Use `pnpm dev:backend` only when intentionally starting or checking the Automatic1111 runtime before opening the UI.
